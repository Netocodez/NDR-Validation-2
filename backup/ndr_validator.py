...
def validate_ndr(data: dict) -> list:
    ...
    for date, enc in data["encounters"].items():
        ...
        # Update expected runout from ARV refill
        if enc.get("arv") and enc.get("duration"):
            try:
                duration = int(enc["duration"])
                last_refill_date = visit_dt
                expected_run_out = visit_dt + timedelta(days=duration)
            except ValueError:
                add_issue(f"⚠️ {date}: Duration is not a valid number.")

        # If encounter occurs after runout but no ARV given, and is not a known lab-only visit
