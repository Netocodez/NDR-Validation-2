...
def extract_services(xml_path: str) -> dict:
    ...
    if hiv_q is not None:
      ...
      if art_start:
          ...
          #print()
          print(f"Flag added: {data.get('validation_flags', [])}")

      # ✅ Add this block to capture height at ART start
      ...
      if height_at_art_start:
          ...
          print("Captured height_at_art_start:", height_at_art_start)
...
def validate_ndr(data: dict) -> list:
    ...
    print("Current validation_flags:", data["validation_flags"])
    issues = []
    ipt_dates = {
    ...
    # Raise flag if Missing ARTStartDate is in validation_flags (or art_start is None)
    ...
    #print("Encounters content:", data.get("encounters"))
    #print("Encounters length:", len(data.get("encounters", {})))
    #print("Current validation_flags3:", data["validation_flags"])
    for date, enc in data["encounters"].items():
        #print("Current validation_flags2:", data["validation_flags"])
        if date == "Unknown":
        ...
        try:
            ...
            if "Missing ARTStartDate" in data.get("validation_flags", []):
                print("Missing ARTStartDate flag is present in validation_flags")
            else:
                print("Missing ARTStartDate flag NOT found")
            if "Missing ARTStartDate" in data.get("validation_flags", []):
              issues.append("❌ ARTStartDate is missing in HIVQuestions section.")          
